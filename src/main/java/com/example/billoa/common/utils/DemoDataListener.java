...
@Slf4j
public class DemoDataListener implements ReadListener<Inbill> {
     ...
     * 缓存的数据
     */
    private List<Inbill> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);
     ...
     *
     */
    public DemoDataListener(InbillServiceImpl inbillService) {
     ...
     */
    @Override
    public void invoke(Inbill data, AnalysisContext context) {
    ...
    private void saveData() {
        ...
        inbillService.save(cachedDataList);
        log.info("存储数据库成功！");
    }
}
